...
public class FormManager {
    ...
    public static void logout() {
        ...
        // Instalar modal system para el logout
        GlassPanePopup.install(instance.frame);
        // Mostrar modal de login moderno
        FlatAnimatedLafChange.hideSnapshotWithAnimation();
        showLoginModal();
    }

    private static void showLoginModal() {
        String icon = "raven/login_register/icon/account.svg";
        ModalDialog.showModal(instance.frame,
                new CustomModalBorder(new Login(), "Bienvenido", icon),
    ...
    private static void closeAllModals() {
        try {
            ...
            // Forzar el cierre de cualquier modal pendiente
            if (ModalDialog.isIdExist(Login.ID)) {
                ModalDialog.closeAllModal();
            }
        } catch (Exception e) {
            // Ignorar errores de cierre de modal
            System.err.println("Error cerrando modal: " + e.getMessage());
        }
    }
